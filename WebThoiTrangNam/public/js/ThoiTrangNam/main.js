$(document).ready(function () {
    ...
    function LoadDataBoLoc() {
        ...
        $.ajax({
            url: 'AoQuanNam', // Đường dẫn API để lấy dữ liệu
            method: 'GET',
            dataType: 'json',
            success: function (data) {
    ...
    function DanhMuc(data) {
        $('.DanhMuc .MenuItem').empty();

        if (data["danhMuc"].length == 0) {
            $('.DanhMuc').hide();
        } else {
            $('.DanhMuc').show();
        }
        ...
        data["danhMuc"].forEach(function (item) {
            ...
            var ItemInput = $('<input type="checkbox" data-item="' + item["data-item"] + '" value="' + item["text"] + '"></input>');
            var ItemP = $('<p>' + item["text"] + '</p>');

            ItemDiv.append(ItemInput, ItemP);

            // Thêm ItemDiv vào .MenuItem trong vòng lặp
            $('.DanhMuc .MenuItem').append(ItemDiv);
        });

    }
    function ThietKe(data) {



        $('.ThietKe .MenuItem').empty();

        data["thietKe"].forEach(function (item) {
            // Tạo các phần tử và thêm chúng vào ItemDiv trong vòng lặp
            var ItemDiv = $('<div class="Item"></div>');
            var ItemInput = $('<input type="checkbox" data-item="thiết kế" value="' + item["thiết kế"] + '"></input>');
            var ItemP = $('<p>' + item["thiết kế"] + '</p>');

            ItemDiv.append(ItemInput, ItemP);

            // Thêm ItemDiv vào .MenuItem trong vòng lặp
            $('.ThietKe .MenuItem').append(ItemDiv);
        });
    ...
    function Form(data) {
        ...
        data["form"].forEach(function (item) {
            // Tạo các phần tử và thêm chúng vào ItemDiv trong vòng lặp
            var ItemDiv = $('<div class="Item"></div>');
            var ItemInput = $('<input type="checkbox" data-item="form" value="' + item["form"] + '"></input>');
            var ItemP = $('<p>' + item["form"] + '</p>');

            ItemDiv.append(ItemInput, ItemP);

            // Thêm ItemDiv vào .MenuItem trong vòng lặp
            $('.Form .MenuItem').append(ItemDiv);
        });
    ...
    function NhomSanPham(data) {

        $('.NhomSanPham .MenuItem').empty();


        // console.log(data["loaiBoLoc"])
        data["loaiBoLoc"].forEach(function (item) {
            // Tạo các phần tử và thêm chúng vào ItemDiv trong vòng lặp
            var loai = item["loại áo"] || item["loại quần"]
            var data;

            if (loai === item["loại áo"]) {
                data = "loại áo";
            } else {
                data = "loại quần";
            }
            ...
            var ItemDiv = $('<div class="Item"></div>');
            var ItemInput = $('<input type="checkbox" data-item="' + data + '" value="' + loai + '"></input>');
            var ItemP = $('<p>' + loai + '</p>');

            ItemDiv.append(ItemInput, ItemP);

            // Thêm ItemDiv vào .MenuItem trong vòng lặp
            $('.NhomSanPham .MenuItem').append(ItemDiv);
        });
    ...
    function ChatLieu(data) {

        $('.ChatLieu .MenuItem').empty();


        data["chatLieu"].forEach(function (item) {
            // Tạo các phần tử và thêm chúng vào ItemDiv trong vòng lặp
            var cl = item["chất liệu"]
            var ItemDiv = $('<div class="Item"></div>');
            var ItemInput = $('<input type="checkbox" data-item="chất liệu" value="' + cl + '"></input>');
            var ItemP = $('<p>' + cl + '</p>');

            ItemDiv.append(ItemInput, ItemP);

            // Thêm ItemDiv vào .MenuItem trong vòng lặp
            $('.ChatLieu .MenuItem').append(ItemDiv);
        });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.message) {
                    ...
                    $('.DanhSachSanPham .thongBao').text(data.message);
    ...
    $(document).ready(function () {
        ...
        KeoChonGia();
        function LoadDuLieuGia() {

            var selectedGia = $('.MenuItemGia .selected-price').text();
            var danhSachLuaChon = $('.DanhSachLuaChon');
            var divButton = $('<div class="button"></div>');
            var p = $('<p data-item="giá tiền" >' + selectedGia + '</p>');
            var i = $('<i class="fas fa-times"></i>');
        ...
        function KeoChonGia() {
            ...
            $('.MenuItemGia .selected-price').
                text(formatCurrency(0) + " - " + formatCurrency(2000000));

            $('.MenuItemGia input[type="range"]').eq(0).on('input', function () {
                // Lấy giá trị của thanh trượt EP0
                var minValue = parseInt($(this).val());
                var maxValue = parseInt($('.MenuItemGia input[type="range"]').eq(1).val());

                // Kiểm tra nếu giá trị EP0 lớn hơn giá trị EP1, cập nhật giá trị EP1
                if (minValue > maxValue) {
                    maxValue = minValue;
                    $('.MenuItemGia input[type="range"]').eq(1).val(maxValue);
                }

                // Cập nhật nội dung của phần tử .selected-price
                // $('.MenuItemGia .selected-price').text(minValue + " - " + maxValue + " VND");
                $('.MenuItemGia .selected-price').
                    text(formatCurrency(minValue) + " - " + formatCurrency(maxValue));
            ...
            $('.MenuItemGia input[type="range"]').eq(1).on('input', function () {
                // Lấy giá trị của thanh trượt EP1
                var maxValue = parseInt($(this).val());
                var minValue = parseInt($('.MenuItemGia input[type="range"]').eq(0).val());

                // Kiểm tra nếu giá trị EP1 nhỏ hơn giá trị EP0, cập nhật giá trị EP0
                if (maxValue < minValue) {
                    minValue = maxValue;
                    $('.MenuItemGia input[type="range"]').eq(0).val(minValue);
                }
            ...
            $('.MenuItemGia input[type="range"]').on('mouseup', function () {
                // Kiểm tra xem có đang kéo không
                if (isDragging) {
                    // Gọi hàm LoadDuLieuGia() khi kết thúc kéo
                    $('.DanhSachLuaChon .button p[data-item="giá tiền"]').closest('.button').remove();
                    LoadDuLieuGia();
